...
/*
  GRAPH2D.I
  Additional functions for graphics written by Hiroshi C. Ito. 2024
...
func scalexy_log(xratio,yratio){
  if(is_void(xratio))xratio=0.1;
  if(is_void(yratio))yratio=0.1;
  lims=limits();
  xr=log(lims(2)/lims(1));
  yr=log(lims(4)/lims(3));
  x0=log(lims(1))-xr*xratio;
  x1=log(lims(2))+xr*xratio;
  y0=log(lims(3))-yr*yratio;
  y1=log(lims(4))+yr*yratio;
  limits,exp(x0),exp(x1),exp(y0),exp(y1);

}

func scalex_log(xratio){
  if(is_void(xratio))xratio=0.1;
  lims=limits();
  xr=log(lims(2)/lims(1));
  x0=log(lims(1))-xr*xratio;
  x1=log(lims(2))+xr*xratio;
  limits,exp(x0),exp(x1),,;

}

func scaley_log(yratio){
  if(is_void(yratio))yratio=0.1;
  lims=limits();
  yr=log(lims(4)/lims(3));
  y0=log(lims(3))-yr*yratio;
  y1=log(lims(4))+yr*yratio;
  limits,,,exp(y0),exp(y1);
...
if(axis==-2){window,n,style=Y_DIR+"gs/my_box_no_labels.gs",width=int(width),height=int(height);}
        ...
        if(axis==6){window,n,style=Y_DIR+"gs/my_test_divide1.gs",width=int(width),height=int(height);}
        if(axis==-6){window,n,style=Y_DIR+"gs/my_test_divide1b.gs",width=int(width),height=int(height);}
    ...
    if(abs(axis)==6){
      ...
      systems.viewport=[[xs2,xe2,ys2,ye2],[xs1,xe1,ys1,ye1]];
    }
    if(axis==7){
      //systems.viewport=[[0.1757,0.25,0.4257,0.8643],[0.25,0.6143,0.4257,0.8643]];
...
func cbfc(z,levs=,colors=,dxy=,w=,h=,tick_value=,tick_label=,tick_col=,tick_width=,dt=)
 ...
 */
{
  ...
  if(!is_void(tick_value)){
    
    if(is_void(dt))dt=0.005;
    if(is_void(tick_col))tick_col=array(black,numberof(tick_value));
    if(is_void(tick_width))tick_width=array(1,numberof(tick_value));
    

      
    dy=(y1-y0)/(max(z)-min(z));
    for(i=1;i<=numberof(tick_value);i++){
      if(tick_width(i)!=-1)plc,z,y,x,levs=[tick_value(i)],color=tick_col(i),width=tick_width(i);
      plg, array(dy*(tick_value(i)-min(z))+y0d,2), [x1+dt,x1],marks=0,color="fg",width=1,type=1;
      if(is_void(tick_label))plt, pr1(tick_value(i)), x1+2*dt,dy*(tick_value(i)-min(z))+y0d, justify="LH",color="black";
      if(!is_void(tick_label))plt, tick_label(i), x1+2*dt,dy*(tick_value(i)-min(z))+y0d, justify="LH",color="black";
...
func plgl(y,x,legend=,color=,type=,width=,position=,font=,n=,font_color=,flip_h=)
 ...
 */
  {
    ...
    extern my_legend_outside,my_legend_side,my_legend_length;
    extern my_legend_marg_b,my_legend_marg_w,my_legend_length;
    ...
    if(dimsof(systems.viewport)(1)==1){
       my_legend_position=[[systems.viewport(1),systems.viewport(4)],[systems.viewport(2),systems.viewport(4)]];
    }
    if(dimsof(systems.viewport)(1)>1){
      my_legend_position=[[systems.viewport(1,plsys_orig),systems.viewport(4,plsys_orig)],[systems.viewport(2,plsys_orig),systems.viewport(4,plsys_orig)]];
    }
...
if(is_void(y)){fma; my_legend_number=0;my_legend_color=-5;return;}
 ...
 if(is_void(font_color))font_color=black;
 if(is_void(flip_h)){
   flip_h=1.0;
 }else{
   flip_h=-1.0;
 ...
 marg_w=my_legend_marg_w;
 marg_b=my_legend_marg_b;
 dleg=my_legend_height*0.0015*flip_h;
 lx=lp(1)+si*marg_w;
 ly=lp(2)+marg_h-my_legend_number*dleg;
 plg,y,x,color=color,type=type,width=width,legend=legend,n=n;
 if((!omit_legend)+(legend!=-1)){
     plsys,0;
        plg,[ly,ly],[lx,lx+si*my_legend_length],color=color,type=type,width=width,legend="";
     //     plg,[ly,ly],[lx,lx+si*0.01],color=color,type=type,width=width,legend="";
     plt,legend,lx+si*(my_legend_length+marg_b),ly,justify=just,color=font_color,height=my_legend_height,font=font;
...
func plmkl(y,x,legend=,color=,marker=,msize=,width=,position=,font=,lmsize=,font_color=)
/* DOCUMENT plmkl,y,x,legend=legend
   DEFINITION plmkl(y,x,legend=,color=,type=,width=,position=,font=)
 ...
 */
  ...
  extern my_legend_number;
  extern my_legend_color;
  extern my_legend_outside,my_legend_side,my_legend_margw,my_legend_margh,my_legend_length,my_legend_marg_b;
  //  l_length=0.02;
    l_length=my_legend_length;
...
if(is_void(y)){fma; my_legend_number=0;my_legend_color=-5;return;}
 if(is_void(color))color=my_legend_color;
 if(is_void(legend))legend="";
 if(is_void(lmsize))lmsize=msize;
 if(is_void(font_color))font_color="black";
 ...
 lx=lp(1)+si*marg_w;
 ly=lp(2)+marg_h-my_legend_number*dleg;
 plmk,y,x,color=color,marker=marker,msize=msize,width=width;
 if(!omit_legend){
     plsys,0;
     lposy=avg([ly,ly]);
     lposx=avg([lx,lx+si*l_length]);
     plmk,lposy,lposx,color=color,marker=marker,msize=lmsize,width=width;
     plt,legend,lx+si*(l_length+marg_b),ly,justify=just,color=font_color,height=my_legend_height,font=font;
...
func plh(y,x,ecol=,width=,fcol=,legend=,type=,baseline=,omit_side=,flip_xy=,font_color=)
/* DOCUMENT plh,y,x
 ...
 */
{
  if(is_void(flip_xy))flip_xy=0;
  if(is_void(baseline))baseline=0.0;
  ...
  if(legend==-1){
    ...
    plgl, fy, fx, color=ecol,width=width,legend=legend,type=type,font_color=font_color;
  }
...
func hcpng (name,dens,geom,geomy,epsi=,outps=,view=,bgcolor=,alpha=,t=)
/* DOCUMENT hapng, name
 ...
 */
{
  ...
  if(is_void(outps))outps=0;
    ...
    if(is_void(geom)*is_void(geomy))system,convert_bin+swrite(format=" -density %ix%i  %s %s",dens,dens,name+suf,name);
    
    if(!is_void(geom)*is_void(geomy))system,convert_bin+swrite(format=" -density %ix%i -resize %ix %s %s",dens,dens,geom,name+suf,name);
    if(!is_void(geomy)*is_void(geom))system,convert_bin+swrite(format=" -density %ix%i -resize x%i %s %s",dens,dens,geomy,name+suf,name);
    if(!is_void(geom)*!is_void(geomy))system,convert_bin+swrite(format=" -density %ix%i -resize %ix%i! %s %s",dens,dens,geom,geomy,name+suf,name);
       ...
       if(alpha==0){
         if(epsi==1)system,convert_bin+swrite(format=" %s -bordercolor %s -border 10x10 -background %s -alpha remove -flatten %s", name,bgcolor,bgcolor,name);
        write,"png output:",name;
        if(epsi==0)system,convert_bin+swrite(format=" %s -background %s -alpha remove -flatten %s", name,bgcolor,name);
...
func cb_dis(levs=,colors=,ticks=,dxy=,w=,h=,sys=,dt=,labels=,linecol=,linewidth=,linetype=)
/* DOCUMENT cb,cmin,cmax
     draw a color bar to the right of the plot by "plf" function
     instead of "pli" function. 
     ("pli" function may cause some problems in output of images.)
 */
{
  ...
  z=y;
    ...
    colors=grow(colors(1),colors);
    ncol=numberof(colors);
    ylevs=((levs-min(levs))/(max(levs)-min(levs)))*(y1-y0)+y0;
    y = transpose(array(ylevs,2));
    x = array(span(x0,x1,2),numberof(ylevs));
    z=colors(-:0:1,2:-1);
  }
  ...
  if(!is_void(labels)){
    if(is_void(dt))dt=0.005;  
    for(i=1;i<=numberof(ylevs);i++){      
      if(labels(i)!="!"){
        
        if((!is_void(linecol)+!is_void(linewidth)+!is_void(linetype))){
          if(is_void(linecol)){
            linec="fg";
          }else{
            linec=linecol(i);
          ...
          if(linec!="!")plg, array(ylevs(i),2), [x0,x1],marks=0,color=linec,width=linew,type=linet;        
        }
        plt, labels(i), x0-2*dt,ylevs(i), justify="RH",color="black";
        plg, array(ylevs(i),2), [x0-dt,x0],marks=0,color="fg",width=1,type=1;        
      }    
    }
  }
