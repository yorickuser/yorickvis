/* The following program has been modified a bit by Hiroshi C. Ito 2018*/
/*
   PL3D.I
...
func _3cage(xyzlim)
{
  ...
  if (!omit_ticks3) {
    ...
    for (m=1 ; m<=3 ; ++m) {
      //###
      if(m==1)mmask=[0,1.0,0];
      if(m==2)mmask=[1.0,0,0];
      if(m==3)mmask=[0,1.0,0];
	...
	tend= xyz0 + xminor;
        tend1= -1*_3lminor*(xyz(,1,n+1,m)-xyz(,1,n,m))*mmask+tend;
	...
	tend= xyz0 + xmajor;
        ...
        tend1= -1*_3lmajor*(xyz(,1,n+1,m)-xyz(,1,n,m))*mmask+tend;
        ...
        plt,label3(m),avg(x2+label3_amp_dist*dmx)+label3off(1,m),avg(y2+label3_amp_dist*dmy)+label3off(2,m),tosys=1,justify="CH",color=_3kcolor,orient=rotate_z,height=label3_height;
...
/* tick parameters */
_3nmajor= 5.5;     /* 1/(max allowed tick density on longest edge) */
_3lmajor= 0.04;    /* length of major ticks as fraction of longest edge */
_3lminor= 0.02;    /* length of minor ticks as fraction of longest edge */
_3ktype= 1;
_3kwidth= 1.0;
_3kcolor= -2;
/* label parameters */
