/*
  PL3D_MODIFIEDF.I
  Modified file of pl3d_modified.i for yorickvisf.i, by Hiroshi C. Ito. 2024
...
func get3_light1(xyz, nxyz)
/* DOCUMENT get3_light1(xyz, nxyz)
         or get3_light1(xyz)
...
//   numberof(nxyz)
//numberof(centroid) 
nxyz_buf = transpose(double([nxyz,nxyz,nxyz]));
centroid = centroid/nxyz_buf;
  }
 ...
 */
{
  if (_draw3_changes) {
      //### changed by hiroshi
      if ((called_as_idler)&&(!omit_draw3_fma)) fma;
    ...
    if (_cage3) {
      extern cage3_front;
      //###
      if ((!is_void(lims))&&(cage3_front==1)) _3cage_front,transpose(lims(1:2,));
    }
...
func _3cage(xyzlim)
{
  ...
  if(structof(cage3_backscreen)==char)plfp, [cage3_backscreen],y1, x1,[numberof(y1)];
  ...
  nlabel= 0;
  extern omit_ticks3,label3_font,ticks3_dirx,ticks3_diry,ticks3_dirz,ticks3_vert_axis,ticks3_horiz_axis_amp,ticks3_horiz_axis;
  if (!omit_ticks3) {
    ...
    [[ip+j+k,ip+j+kp],[i+jp+k,i+jp+kp]]];
     ...
     //list= [[[i+j+k,ip+j+k],[i+jp+k,ip+jp+k]],
     //[[i+j+k,i+jp+k],[ip+j+k,ip+jp+k]],
     //[[ip+j+k,ip+j+kp],[i+jp+k,i+jp+kp]]];
    ...
    if(ticks3_horiz_axis_amp>0){
      horiz= (abs(y1(ptp,,))> ticks3_horiz_axis_amp*abs(x1(ptp,,)));
    }else{
      horiz= (abs(y1(ptp,,))>abs(x1(ptp,,)));
    }
    ...
    x2= x1(avg,,);
    y2= y1(avg,,);
    llen=sqrt(x1(ptp,,)^2+y1(ptp,,)^2);
    llen-=(llen(avg,))(-:1:2,);

    amp_vert=sign(x2-x2(2:1:-1,));
    amp_horiz=sign(y2-y2(2:1:-1,));
  
    if(ticks3_vert_axis=="right")amp_vert=-sign(x2-x2(2:1:-1,));
    if(ticks3_vert_axis=="close")amp_vert=-sign(llen);
    if(ticks3_hori_axis=="top")amp_horiz=-sign(y2-y2(2:1:-1,));

    dir= amp_vert*horiz + amp_horiz*(!horiz);
    ...
    unit(1:9:4,1)= 1.;
    llen= [0,0,0];
    xptr= yptr= lptr= xptr0=yptr0=array(pointer, 3);
    ...
    for (m=1 ; m<=3 ; ++m) {
      ...
      if(m==1)mmask=ticks3_dirx;
      if(m==2)mmask=ticks3_diry;
      if(m==3)mmask=ticks3_dirz;
	 ...
	 major ticks -- other two edges get scaled number */
      ...
      if (_3ticks(xyzlim(m,1), xyzlim(m,2), _3nmajor(m)*len(m)))
	continue;        /* (no ticks on very short axes) */
      ...
      /* ### I changed to disapper one side of ticks*/
      for (n=1 ; n>=1 ; --n) {
	xyz0= xyz(,1,n,m)*mask;
	...
	tend= xyz0 + xminor;
        tend1= -1*_3lminor(m)*(xyz(,1,n+1,m)-xyz(,1,n,m))*mmask+tend;
        //tend1_grid= (xyz(,1,n+1,m)-xyz(,1,n,m))*mmask+tend;

        get3_xy, tend, x1, y1;
        get3_xy, tend1, x2, y2;
        //get3_xy, tend1_grid, x2_grid, y2_grid;
        
	pldj, x1,y1,x2,y2, legend="",type=_3ktype,color=_3kcolor,width=_3kwidth;
        //if(m!=3)pldj, x1,y1,x2_grid,y2_grid, legend="",type=2,color=_3kcolor,width=_3kwidth;
	...
	tend= xyz0 + xmajor;
        ...
        tend1= -1*_3lmajor(m)*(xyz(,1,n+1,m)-xyz(,1,n,m))*mmask+tend;

        get3_xy, tend, x1, y1,z1,1;
        get3_xy, tend1, x2, y2,z2,1;
        dmx=x2-x1;
        dmy=y2-y1;

        if (!omit_label3){
          extern z_vertical,label3,label3off,label3_height,label3_amp_dist;
        if((m==3)&&(z_vertical==1))rotate_z=1;
        else rotate_z=0;
        //plt,label3(m),avg(x2)+0.2*dmx(m)+label3off(1,m),avg(y2)+0.2*dmy(m)+label3off(2,m),tosys=1,justify="CC",color=_3kcolor,orient=rotate_z;
        plt,label3(m),avg(x2+label3_amp_dist*dmx)+label3off(1,m),avg(y2+label3_amp_dist*dmy)+label3off(2,m),tosys=1,justify="CH",color=_3kcolor,orient=rotate_z,height=label3_height,font=label3_font;
      
        
        //plt,label3(m),avg(x2)+0.2*dmx(m),avg(y2)+0.2*dmy(m),tosys=1,justify="CC",color=_3kcolor;  
        }
      ...
      x2=x2+(x2-x1)*_3xmargin(m);
      y2=y2+(y2-y1)*_3xmargin(m);
      xptr(m)= &x2;
      yptr(m)= &y2;
      //xptr0(m)= &x1;
      //yptr0(m)= &y1;
   ...
   if (!omit_labels && nlabel) {
	...
	eq_nocopy, x2, *xptr(m);
	eq_nocopy, y2, *yptr(m);
        eq_nocopy, x1, *xptr0(m);
	eq_nocopy, y1, *yptr0(m);
	...
	if (numberof(xlabel)) {
          ...
          //          x2+=(x2-x1)*1;
          //y2+=(y2-y1)*1;
          //plmk,y2,x2,msize=0.2,marker=3;            
             
            //  	  if (horiz) x2+= offset;
            //else       y2+= offset;   /* add term in thoriz(m)? */
            //if (horiz) justify= (tdir<0.)? 10 : 13;
            //else       justify= (tdir<0.)? 10 : 18;
          
          justify="CH";          
	  for (n=1 ; n<=numberof(xlabel) ; ++n) {
	    plt, xlabel(n), x2(n),y2(n), tosys=1, justify=justify,color=_3xcolor,font=_3xfont,height=_3xheight;
...
/* tick parameters */
_3nmajor= [5.5,5.5,5.5];     /* 1/(max allowed tick density on longest edge) */
_3lmajor= [0.04,0.04,0.04];    /* length of major ticks as fraction of longest edge */
_3lminor= [0.02,0.02,0.02];    /* length of minor ticks as fraction of longest edge */
_3ktype= 1;
_3kwidth= 1.0;
_3kcolor= -2;
/* label parameters */
_3xmargin=[1.5,1.5,1.5];
_3xoffset= 0.02;   /* label offset from tick as fraction of longest edge */
_3xfudge= 1.0;     /* fudge factor for points/(longest edge) calc */
//_3xfont= 3;        /* helvetica */
_3xfont= 8;        /* helvetica */

_3xheight= 10.0    /* point size of label text */
_3xcolor= -2;
